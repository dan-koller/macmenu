...
/*
 * After a window has been moved and resized, if the window could not be resized small enough to fit 
 * the intended size, then some of the window may appear off the screen. The BestEffortWindowMover
 * will move the window so that it fits entirely on the screen.
 */
...
class BestEffortWindowMover: WindowMover {
    func moveWindowRect(_ windowRect: CGRect, frameOfScreen: CGRect, visibleFrameOfScreen: CGRect, frontmostWindowElement: AccessibilityElement?, action: WindowAction?) {
        ...
        if adjustedWindowRect.minX < visibleFrameOfScreen.minX {
            ...
            adjustedWindowRect.origin.x = visibleFrameOfScreen.minX + visibleFrameOfScreen.width - (adjustedWindowRect.width) - CGFloat(Constants.gapSize.value)
        }
        ...
        if adjustedWindowRect.minY < visibleFrameOfScreen.minY {
            ...
            adjustedWindowRect.origin.y = visibleFrameOfScreen.minY + visibleFrameOfScreen.height - (adjustedWindowRect.height) - CGFloat(Constants.gapSize.value)
        }
