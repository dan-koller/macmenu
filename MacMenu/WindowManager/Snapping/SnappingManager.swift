...
class SnappingManager {
    ...
    private let fullIgnoreIds: [String] = Constants.fullIgnoreBundleIds.typedValue ?? ["com.install4j", "com.mathworks.matlab", "com.live2d.cubism.CECubismEditorApp", "com.aquafold.datastudio.DataStudio"]
    ...
    private let marginLeft = Constants.snapEdgeMarginLeft.cgFloat
    private let marginRight = Constants.snapEdgeMarginRight.cgFloat
    
    init() {
        if Constants.windowSnapping.enabled != false {
            enableSnapping()
        }
    ...
    func toggleListening() {
        if allowListening, !isFullScreen, !Constants.windowSnapping.userDisabled {
            enableSnapping()
        } else {
            disableSnapping()
        }
    ...
    public func reloadFromDefaults() {
        if Constants.windowSnapping.userDisabled {
            if eventMonitor?.running == true {
                disableSnapping()
            }
        } else {
            if eventMonitor?.running == true {
                if Constants.missionControlDragging.userDisabled != (eventMonitor is ActiveEventMonitor) {
                    stopEventMonitor()
                    startEventMonitor()
                }
            } else {
    ...
    private func startEventMonitor() {
        ...
        eventMonitor = Constants.missionControlDragging.userDisabled ? ActiveEventMonitor(mask: mask, filterer: filter, handler: handle) : PassiveEventMonitor(mask: mask, handler: handle)
        eventMonitor?.start()
    }
        ...
        case .leftMouseDragged:
            if let cgEvent = event.cgEvent, let screen = NSScreen.main {
                ...
                if cgEvent.location.y == minY && dragPrevY == minY {
                    if event.deltaY < -Constants.missionControlDraggingAllowedOffscreenDistance.cgFloat {
                        cgEvent.location.y = minY + 1
                        dragRestrictionExpirationTimestamp = DispatchTime.now().uptimeMilliseconds + UInt64(Constants.missionControlDraggingDisallowedDuration.value)
    ...
    func canSnap(_ event: NSEvent) -> Bool {
        if Constants.snapModifiers.value > 0 {
            if event.modifierFlags.intersection(.deviceIndependentFlagsMask).rawValue != Constants.snapModifiers.value {
                return false
            }
        }
    ...
    func handle(event: NSEvent) {
        switch event.type {
        case .leftMouseDown:
            if !Constants.obtainWindowOnClick.userDisabled {
            ...
            if windowMoving {
                ...
                if let snapArea = snapAreaContainingCursor(priorSnapArea: currentSnapArea) {
                    ...
                    if let newBoxRect = getBoxRect(hotSpot: snapArea, currentWindow: currentWindow) {
                        ...
                        if Constants.footprintAnimationDurationMultiplier.value > 0 {
    ...
    func unsnapRestore(windowId: CGWindowID, currentRect: CGRect, cursorLoc: CGPoint?) {
        if Constants.unsnapRestore.enabled != false {
    ...
    func getFootprintAnimationDuration(_ box: FootprintWindow, _ boxRect: CGRect) -> Double {
        return box.animationResizeTime(boxRect) * Double(Constants.footprintAnimationDurationMultiplier.value)
    }
    ...
    func getBoxRect(hotSpot: SnapArea, currentWindow: Window) -> CGRect? {
        if let calculation = WindowCalculationFactory.calculationsByAction[hotSpot.action] {
            ...
            let rectCalcParams = RectCalculationParameters(window: currentWindow, visibleFrameOfScreen: hotSpot.screen.adjustedVisibleFrame(true), action: hotSpot.action, lastAction: nil)
            ...
            if Constants.gapSize.value > 0, gapsApplicable != .none {
                ...
                return GapCalculation.applyGaps(rectResult.rect, dimension: gapsApplicable, sharedEdges: gapSharedEdges, gapSize: Constants.gapSize.value)
            }
    ...
    func directionalLocationOfCursor(loc: NSPoint, screen: NSScreen) -> Directional? {
        ...
        let cornerSize = Constants.cornerSnapAreaSize.cgFloat
